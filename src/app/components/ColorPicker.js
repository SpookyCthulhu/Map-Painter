...
const ColorPicker = ({ setColor }) => {
  return (
    <div className="fixed bottom-4 right-8 bg-white bg-opacity-40 shadow-lg p-4 rounded-lg w-120 h-16 flex items-center">
      ...
      <div className="flex-1 flex justify-center items-center space-x-2 p-2">
        ...
        <div className="w-8 h-8 bg-orange-500 cursor-pointer" onClick={() => setColor('orange')}></div>
      ...
      <div className="size-8">
        <input
          ...
          className="w-8 h-8 p-0 border-2 cursor-pointer rounded-3xl"
        />
      </div>
    </div>
  );
